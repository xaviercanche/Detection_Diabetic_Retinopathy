...
def etiquetas( name ):
    ...
    label_I = label(I)
    propiedades = regionprops(label_I,properties={'coords','centroid'}, cache=True)

    for i in range(len(propiedades)):
        if Mask[np.split(np.array(propiedades[i].centroid).astype(int),2)] >= NIVEL*255:
            # Calculamos el numero de pixeles que se solapan
            A_region = np.sum(Mask[np.split(np.array(propiedades[i].coords),2,axis=1)])
            # Calculamos el Area de la Marca del Experto
            A_exudate = np.sum(label_I == label_I[np.split(np.array(propiedades[i].centroid).astype(int),2)])
            if float(A_region)/A_exudate >= 0.75:
                E += [1] # Exudado Duro
            else:
                E += [0]
        else:
